...
final class BootUtil {

	private static final Logger logger = Logger.getLogger(BootUtil.class
			.getName());
	...
	static void restoreSystemProperties() {
		...
		for (Map.Entry<Object, Object> entry : props.entrySet()) {
			...
			if (System.getProperties().containsKey(key)) {
				...
				logger.log(Level.INFO, "override system property " + key
						+ " from " + originalValue + " to " + value);
			} else {
				logger.log(Level.FINER, "set system property " + key + " = "
						+ value);
			}
	...
	private static Properties loadPropertiesFromClasspath(final String path) {
		...
		try {
			...
			logger.log(Level.SEVERE, "load properties from classpath \"" + path
					+ "\" failed", e);
		}
		return props;
	}
