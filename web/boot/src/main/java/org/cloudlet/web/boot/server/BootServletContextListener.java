...
public final class BootServletContextListener extends
		GuiceServletContextListener {

	private ServiceLoader<ServletContextListener> servletContextListeners;
	private static final Logger logger = Logger
			.getLogger(BootServletContextListener.class.getName());

	@Override
	public void contextDestroyed(final ServletContextEvent servletContextEvent) {
		super.contextDestroyed(servletContextEvent);
		if (servletContextListeners != null) {
			Iterator<ServletContextListener> it = servletContextListeners
					.iterator();
			while (it.hasNext()) {
				ServletContextListener listener = it.next();
				logger.finer("Destroy ServletContextListener: "
	...
	public void contextInitialized(final ServletContextEvent servletContextEvent) {
		...
		if (servletContextListeners != null) {
			...
			while (it.hasNext()) {
				ServletContextListener listener = it.next();
				logger.finer("Initialize ServletContextListener: "
						+ listener.getClass().getName());
