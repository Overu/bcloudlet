...
public class JpaFinderProxy implements MethodInterceptor {
	...
	private static class FinderDescriptor {
		...
		// contract is: null = no bind, @Named = param, @FirstResult/@MaxResults
		// for paging
		...
		Query createQuery(final EntityManager em, final Object[] arguments) {
			...
			// List<SortPredicateProxy> sorts = (List<SortPredicateProxy>)
			// arguments[sortIdx];
			// StringBuilder sort = new StringBuilder(" order by");
			// for (SortPredicateProxy s : sorts) {
			// sort.append(s.getPropertyName());
			// sort.append(" ");
			// sort.append(SortDirection.ASCENDING == s.getDirection() ? " asc "
			// : " desc ");
			// }
			String ql = query.replace(SortPredicateProxy.SORT,
					arguments[sortIdx] == null ? " " : " order by "
							+ arguments[sortIdx]);
	...
	private final Map<Method, FinderDescriptor> finderCache = new MapMaker()
			.weakKeys().makeMap();
	...
	public Object invoke(final MethodInvocation methodInvocation)
		...
		if (finderDescriptor.isBindAsRawParameters) {
			bindQueryRawParameters(jpaQuery, finderDescriptor,
					methodInvocation.getArguments());
		} else {
			bindQueryNamedParameters(jpaQuery, finderDescriptor,
					methodInvocation.getArguments());
		}
	...
	private void bindQueryNamedParameters(final Query jpaQuery,
			final JpaFinderProxy.FinderDescriptor descriptor,
	...
	private void bindQueryRawParameters(final Query jpaQuery,
		...
		for (int i = 0, index = 1; i < arguments.length; i++) {
			...
			if (null == annotation) {
				// bind it as a raw param (1-based index, yes I know its
				// different from Hibernate, blargh)
	 ...
	 */
	private void cacheFinderDescriptor(final Method method,
			final FinderDescriptor finderDescriptor) {
		// write to concurrent map
		finderCache.put(method, finderDescriptor);
	}

	private JpaFinderProxy.ReturnType determineReturnType(
	...
	private Object getAsCollection(
			final JpaFinderProxy.FinderDescriptor finderDescriptor,
			final List results) {
		Collection<?> collection;
		try {
			collection = (Collection) finderDescriptor.returnCollectionTypeConstructor
					.newInstance();
	...
	private JpaFinderProxy.FinderDescriptor getFinderDescriptor(
			final MethodInvocation invocation) {
		Method method = invocation.getMethod();
		JpaFinderProxy.FinderDescriptor finderDescriptor = finderCache
				.get(method);
			...
			for (Annotation annotation : annotations) {
				...
				if (Named.class.equals(annotationType)
					...
					if (finderDescriptor.query == null
							|| !finderDescriptor.query
									.contains(SortPredicateProxy.SORT)) {
						logger.warning("缺少排序字符串" + SortPredicateProxy.SORT
								+ ": " + finderDescriptor.query);
					}
		...
		if (JpaFinderProxy.ReturnType.COLLECTION
				.equals(finderDescriptor.returnType)
		...
		// if (base == List.class) {
		// Type parameterization = TypeUtils.getSingleParameterization(base,
		// type);
		// if (parameterization != null
		// && TypeUtils.ensureBaseType(parameterization) ==
		// SortPredicateProxy.class) {
		// if (finderDescriptor.sortIdx != -1) {
