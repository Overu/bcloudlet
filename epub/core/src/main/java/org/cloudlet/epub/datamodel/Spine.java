...
import org.cloudlet.epub.util.StringUtil;
...
public class Spine implements Serializable {
	...
	public Spine(TableOfContents tableOfContents) {
		this.spineReferences = createSpineReferences(tableOfContents
	...
	/**
	 * As per the epub file format the spine officially maintains a reference to
	 * the Table of Contents. The epubwriter will look for it here first,
	 * followed by some clever tricks to find it elsewhere if not found. Put it
	 * here to be sure of the expected behaviours.
	 * 
	 * @param tocResource
	 */
	...
	/**
	 * The resource containing the XML for the tableOfContents. When saving an
	 * epub file this resource needs to be in this place.
	 * 
	 * @return
	 */
	...
	public int getResourceIndex(String resourceHref) {
		...
		for (int i = 0; i < spineReferences.size(); i++) {
			if (resourceHref.equals(spineReferences.get(i).getResource()
					.getHref())) {
				result = i;
				break;
			}
