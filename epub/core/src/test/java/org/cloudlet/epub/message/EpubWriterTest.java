...
import org.cloudlet.epub.datamodel.Author;
import org.cloudlet.epub.datamodel.Book;
import org.cloudlet.epub.datamodel.GuideReference;
...
public class EpubWriterTest extends TestCase {
	...
	public void testBook1() {
		try {
			...
			assertEquals(book.getMetadata().getTitles(), readBook.getMetadata()
					.getTitles());
			assertEquals(
					Identifier.Scheme.ISBN,
					CollectionUtil.first(
							readBook.getMetadata().getIdentifiers())
							.getScheme());
			assertEquals(
					CollectionUtil.first(book.getMetadata().getIdentifiers())
							.getValue(),
					CollectionUtil.first(
							readBook.getMetadata().getIdentifiers()).getValue());
			assertEquals(CollectionUtil.first(book.getMetadata().getAuthors()),
					CollectionUtil.first(readBook.getMetadata().getAuthors()));
			assertEquals(
					1,
					readBook.getGuide()
							.getGuideReferencesByType(GuideReference.COVER)
							.size());
	...
	/**
	 * Test for a very old bug where epublib would throw a NullPointerException
	...
	private Book createTestBook() throws IOException {
		...
		book.getMetadata().addIdentifier(
				new Identifier(Identifier.Scheme.ISBN, "987654321"));
		book.getMetadata().addAuthor(new Author("Joe", "Tester"));
		book.setCoverPage(new Resource(this.getClass().getResourceAsStream(
				"/book1/cover.html"), "cover.html"));
		...
		book.addResource(new Resource(this.getClass().getResourceAsStream(
				"/book1/book1.css"), "book1.css"));
		TOCReference chapter2 = book.addSection("Second chapter", new Resource(
				this.getClass().getResourceAsStream("/book1/chapter2.html"),
				"chapter2.html"));
		book.addResource(new Resource(this.getClass().getResourceAsStream(
				"/book1/flowers_320x240.jpg"), "flowers.jpg"));
		book.addSection(chapter2, "Chapter 2 section 1", new Resource(this
				.getClass().getResourceAsStream("/book1/chapter2_1.html"),
				"chapter2_1.html"));
		book.addSection("Chapter 3", new Resource(this.getClass()
				.getResourceAsStream("/book1/chapter3.html"), "chapter3.html"));
		return book;
	}
	...
	// Book book = new Book();
	//
	// Resource coverImg = new Resource(new
	// FileInputStream(ResourceBundle.getBundle("info.pxdev.pfi.webclient.resources.Config").getString("COVER_DIR")+dto.getCoverFileName()),dto.getCoverFileName());
	//
	...
	// dto.getIdentifier().getIdentifier()));
	//
	// book.getMetadata().addAuthor(new Author(dto.getCreator().getName(),
	// dto.getCreator().getLastName()));
	...
	// Author contributor = new Author(contrib.getName(),
	// contrib.getLastName());
	...
	// Resource aux = new
	// Resource(HTMLGenerator.generateChapterHtmlStream(dto,chapter),
	...
	// FileOutputStream(ResourceBundle.getBundle("info.pxdev.pfi.webclient.resources.Config").getString("HTML_CHAPTERS")+dto.getId_book()+"\\test.epub");
	//
	// try {
