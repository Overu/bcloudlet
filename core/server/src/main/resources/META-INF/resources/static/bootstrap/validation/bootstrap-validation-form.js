...
(function($) {       
   ...
   $.fn.validateBlurForm = function(){validateBlur()};
   var defaults = {
        validRules : [
			...
			{name: 'mobile', validate: function(value) {return (!/^(13|15|18)[0-9]{9}$/.test(value));}, defaultMsg: '请输入正确手机号码。'},
			...
			{name: 'decimal', validate: function(value) {return (!/^(\-)?(([0-9])+(\.)([0-9])+|0)$/.test(value))&&(!/^[0-9]\d*$/.test(value));}, defaultMsg: '请输入浮点数字。'},
        ]
    };
...
var validateBlur = function(){
	...
	var el = $(this),valid = (el.attr('check-type')==undefined)?null:el.attr('check-type').split(' ');
		...
		if(valid!==null && valid.length>0){
			//为每个文本绑定获取焦点事件
			el.focus(function(){
				var curTextDiv=el.parent().parent(), curErrorEl = curTextDiv.children('.help-inline')
...
var validateField = function(field,valid){
	...
	var curTextDiv=el.parent().parent(), curErrorEl = curTextDiv.children('.help-inline');
	var curGroupDiv = el.parent().parent().parent();
	//添加/删除 错误描述信息
	if(error){
		//判断当前DIV 是否有错误信息的class
		if(curErrorEl.hasClass('help-inline')){
		...
		el.removeClass().addClass('error');
		 curGroupDiv.removeClass('success').addClass('error');
		...
		curErrorEl.remove();
		el.removeClass().addClass('right');
		curGroupDiv.removeClass('error').addClass('success');
	}
